...
UI Automation project to run tests with [Cucumber](https://cucumber.io/) and brings **true** [BDD](http://en.wikipedia.org/wiki/Behavior-driven_development) to JavaScript. Instead of writing complicated test code that only developers can understand, Cucumber maps an ordinary language to code and allows to start with the test process in the early stages of your product development.

## Requirements

- Node version 8 or higher [https://nodejs.org/en/download/]
- A preconfigured Selenium Grid, preinstalled browser driver or cloud provider account

Although this project works fine with NPM we recommend to use Yarn (>= 1.0.0) instead,  due to its speed & solid dependency locking mechanism. To keep things simple we use yarn in this guide, but feel free to replace this with NPM if that is what you are using.[Yarn](https://classic.yarnpkg.com/en/docs/install/), [Node](https://nodejs.org/en/download/)
...
1. While running ‘yarn install’ If you face any issue related to fibres package or node-gyp then follow below steps

    ▪   Run ’npm install - -global - -production node-gyp’
    ▪   Run ’npm install - -global - -production windows-build-tools’
    ▪   Note: If windows-build-tools command run hangs for more that 10 min then close that cmd/power shell and follow below steps
    ▪   Install chocolatey by following steps mention in below url
    ▪   https://jcutrer.com/windows/install-chocolatey-choco-windows10
    ▪   After chocolatey installed run ‘choco install python visualstudio2017-workload-vctools -y’ as a admin in power shell 
    ▪   Then run ‘npm config set msvs_version 2017’
    ▪   Then run ‘npm config list’. Make sure python and msgs version paths are set
    ▪   after above steps are followed please run ‘yarn install’  and check. If you still face the issues then please follow the below url.
    ▪   http://tutorialspots.com/install-node-gyp-on-windows-4027.html
...
`./tests/features/*` and have the file ending `.feature`. You will already find some test files in that
...
sometimes it's usefult to only execute particular test suite, to do so use the following command:

```sh
$ npx run wdio wdio.conf.js --suite <suiteName>
```
run multiple suites at once:

```sh
$ npx wdio wdio.conf.js --suite <suiteName1> --suite <suiteName2>
```
Note: These suite names should be defined in wdio.conf.js file 
...
```sh
$ npx wdio wdio.conf.js --spec ./test/features/select.feature --spec ./test/features/select2.feature
```
...
- `I expect that shopping cart is empty` <br> check shopping cart is emprt or not If not then do it empty
...
- `I expect that file "([^"]*)?" is( not)* existed` <br>Check If the file is existed in the location or not
- `I expect that file "([^"]*)?"( not)* contains the same content as file "([^"]*)?" delta file "([^"]*)?"` <br>Compare two csv files and generate a delta file If ay differences found

## When steps

- `I (click|doubleclick) on the (link|button|element) "([^"]*)?"` <br>(Double)click a link, button or element
- `I (add|set|enter) "([^"]*)?" to the inputfield "([^"]*)?"` <br>Add or set the content of an input field
...
- `I execute "([^"]*)?" api request "([^"]*)?" with "([^"]*)?" with "([^"]*)?"` <br>Execute api request
- `I delete the file "([^"]*)?"` <br>Delete the file from the location
- `I focus the iframe "([^"]*)?"` <br>Focus the iFrame with selector
- `I focus the parent iframe` <br>Focus back to parent iFrame
- `I upload file "([^"]*)?" using choose file option "([^"]*)?` <br> upload File using file path and file input selector
